...
import { Loader2, Send, Instagram, Twitter, Youtube, MessageSquare } from 'lucide-react'
...
const platformBadge: Record<string, { icon: typeof Instagram; className: string }> = {
  ...
  telegram: { icon: Send, className: 'text-blue-400 bg-blue-500/10' },
}

type InboxView = 'comments' | 'telegram-dms'
...
export function SocialInbox() {
  ...
  const [view, setView] = useState<InboxView>('comments')
  ...
  return (
    <div>
          ...
          <div className="flex gap-1 rounded-lg border border-white/[0.08] bg-white/[0.02] p-0.5">
            <button
              ...
              className={cn(
                ...
                view === 'comments' ? 'bg-white/[0.1] text-zinc-100' : 'text-zinc-500 hover:text-zinc-300'
              )}
            >
              <MessageSquare size={12} /> Comments
            </button>
            <button
              onClick={() => setView('telegram-dms')}
              className={cn(
                'flex items-center gap-1.5 rounded-md px-3 py-1 text-xs font-medium transition-colors',
                view === 'telegram-dms' ? 'bg-white/[0.1] text-zinc-100' : 'text-zinc-500 hover:text-zinc-300'
              )}
            >
              <Send size={12} /> Telegram DMs
            </button>
          </div>

          {/* Comment filters (only for comments view) */}
          {view === 'comments' && (
      ...
      {view === 'telegram-dms' && <TelegramDMInbox />}
