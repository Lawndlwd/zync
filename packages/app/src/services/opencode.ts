...
export async function checkConnection(serverUrl?: string): Promise<OpenCodeConnectionStatus> {
  try {
    const c = getOpenCodeClient(serverUrl)
    await c.config.get()
    return { connected: true, serverUrl: serverUrl || currentServerUrl }
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Cannot connect to OpenCode server'
    return {
      connected: false,
...
export async function fetchProviders(serverUrl?: string): Promise<OpenCodeProvider[]> {
  const c = getOpenCodeClient(serverUrl)
