...
export function HabitBreakdown() {
  ...
  const sorted = [...habitStats].sort((a, b) =>
    sortBy === 'streak' ? b.currentStreak - a.currentStreak : b.completionRate - a.completionRate,
  )
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <Button variant="ghost" size="sm" onClick={() => setSortBy(sortBy === 'streak' ? 'rate' : 'streak')}>
          <ArrowUpDown size={18} />
      ...
      <CardContent>
        <div className="space-y-3">
          {sorted.map(({ habit, currentStreak, bestStreak, completionRate, sparkline }) => (
            <div key={habit.id} className="flex items-center gap-3 rounded-lg bg-card border border-border p-4">
              <HabitIcon name={habit.icon} size={24} className="text-foreground shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-base font-medium text-foreground truncate">{habit.name}</p>
              ...
              <div className="flex items-end gap-1 h-7">
                {sparkline.map((val, i) => (
                  <div
                    key={i}
                    className={cn('w-1.5 rounded-sm', val ? 'bg-primary' : 'bg-muted-foreground')}
